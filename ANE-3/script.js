...
/* ------------------------------
   06 / EL RECORRIDO - CONTROL POR CLASES
------------------------------ */
/* ------------------------------
(() => {
  const section = document.querySelector(".cookie-route-section");
  if (!section) return;
  ...
  function updateRouteClasses() {
    ...
    /*
      Ajustes principales:
      - shortTitle: cuándo aparece "El recorrido corto"
      - short1/2/3: cuándo aparecen los pasos del recorrido corto
      - longTitle: cuándo aparece "El recorrido largo"
      - long1...: cuándo aparecen sus pasos
    */
/*
    const shortTitle = 0.44;
    const shortStep1 = 0.56;
    const shortStep2 = 0.66;
    const shortStep3 = 0.76;

    const longTitle = 0.84;
    const longStep1 = 0.88;
    const longStep2 = 0.91;
    const longStep3 = 0.94;
    const longStep4 = 0.965;
    const longStep5 = 0.985;
  ...
  window.addEventListener("scroll", updateRouteClasses, { passive: true });
  window.addEventListener("resize", updateRouteClasses);
  window.addEventListener("load", updateRouteClasses);
...
/* ------------------------------
   06 / EL RECORRIDO - CABECERA SEPARADA
------------------------------ */
...
(() => {
  ...
  function updateCookieRoute() {
    ...
    const totalScroll = Math.max(rect.height - viewportHeight, 1);
    const currentScroll = routeClamp(-rect.top, 0, totalScroll);
    const progress = currentScroll / totalScroll;

    /*
      El progreso empieza en .cookie-route-stage,
      no en toda la sección 06.
      Así primero pasa el título y después empieza la animación.
    */

    const shortTitle = 0.08;
    const shortStep1 = 0.20;
    const shortStep2 = 0.32;
    const shortStep3 = 0.44;

    const longTitle = 0.58;
    const longStep1 = 0.66;
    const longStep2 = 0.73;
    const longStep3 = 0.80;
    const longStep4 = 0.87;
    const longStep5 = 0.94;
    const captionStep = 0.975;
